...
public class Doorway : MonoBehaviour
{
    ...
    private void Start()
    {
        if (Application.isPlaying) player = GameManager.instance.player.transform;
    }
    ...
    void ShowOpenPrompt()
    {
        var prompt = GameManager.instance.insideLocation ? openInterior : openPrompt;
        prompt.SetActive(true);
    }

    void ShowEnterPrompt()
    {
        var prompt = GameManager.instance.insideLocation ? enterInterior : enterPrompt;
        prompt.SetActive(true);
    }
    ...
    IEnumerator TraverseDoor()
    {
        ...
        GameManager.instance.player.SetPaused(true);
        GameManager.instance.wipe.DoWipe(0.5f, 0.1f, 0.5f);
        yield return new WaitForSeconds(0.5f);

        var pos = GameManager.instance.insideLocation ? outsidePlacement : insidePlacement;
        ...
        GameManager.instance.player.SetPaused(false);

        GameManager.instance.insideLocation = !GameManager.instance.insideLocation;
        if (GameManager.instance.insideLocation) GameManager.instance.EnterNewLocation(locationName);
        moving = false;
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(transform.position + insidePlacement, 0.5f);
        Gizmos.DrawWireSphere(transform.position + outsidePlacement, 0.5f);
    }
}
