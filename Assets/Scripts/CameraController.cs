...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (Application.isPlaying && !mouseEnabled) {
            ...
            controllerScript = GameManager.instance.player;
            LockMouse();
        }
    }
    void LateUpdate()
    ...
    void ProcessMouseInput()
    {
        ...
        if (followWhenFlying && controllerScript.flying) {
            return;
        }
        inputs.y += horizontal;
        inputs.x += vertical;
    ...
    void MoveCam()
    {
        ...
        if (followWhenFlying && controllerScript.flying) {
            ...
            float percentFromMiddle = (Mathf.Abs(inputs.x - defaultX) / largestLimitDist) * (controllerScript.flying ? 0 : 1);
        ...
        controllerScript.cameraRot = cameraParent.transform.localEulerAngles;
    }
